	...
	public static int askCurrentPlayerForTrade() throws IOException {
		...
		if (menuSelect == 1) {
			...
			if (selectWhatToTrade == 2) {
				boolean decideToTradeJailCard = ConsoleUI.promptForBool("Would you like to trade your Get Out of Jail Card?" + "[Y/N]", "Y", "N");
			...
			if (selectWhatToTrade == 3) {
				boolean decideToTradeProperty = ConsoleUI.promptForBool("Would you like to trade some property?" + "[Y/N]", "Y","N");
				...
				if (decideToTradeProperty == true) {
					...
					while (decideTrade) {
						...
						if (continuePropTrade == true) {
							...
							int landToTrade = ConsoleUI.promptForInt("Select what property you want to trade and remove from your inventory", 1,tradingLand.size());
				...
				exitTrade = false;
//				exitTrade = ConsoleUI.promptForBool("Are you sure you want to quit trade?" + "[Y/N]", "Y", "N");
	...
	public static int selectWhatOtherPlayerTrades() throws IOException {
		...
		System.out.println("What would you like to take from Player:" + whatPlayerToTradeWith(inputPlayerNumber, player));
		...
		if (requestOtherPlayerTrade == 1) {
			boolean exchangeMoneyWithOtherPlayer = ConsoleUI.promptForBool("Would you like to request a money exchange?" + "[Y/N]", "Y", "N");
			...
			if (exchangeMoneyWithOtherPlayer == true) {
				...
				if (otherPlayersMoney < enter2Value) {
					System.out.println("Error! Player: " + whatPlayerToTradeWith(inputPlayerNumber, player) + " doesn't have that money money in their account. Try Again.");
		...
		if (requestOtherPlayerTrade == 2) {
			...
			if (askPlayerForJailCard == true) {
				...
				if (!Player.getGetOutOfJailChance(player) && !Player.getGetOutOfJailChest(player)) {
					System.out.println("Sorry, Player: " + whatPlayerToTradeWith(inputPlayerNumber, player) + " doesn't possess a Get out of jail free card");
		...
		if (requestOtherPlayerTrade == 3) {
			boolean requestToTradePlayerProperty = ConsoleUI.promptForBool("Would you like to request some property?" + "[Y/N]","Y", "N");
			...
			if (requestToTradePlayerProperty == true) {
				...
				while (tradeCon) {
					...
					if (continuePropTrade == true) {
						...
						int requestLand = ConsoleUI.promptForInt("Select what property you want to take", 1,tradingPlayerLand.size());
	...
	public static boolean acceptOrDeclinePlayerTradeRequest() throws IOException {
		...
		boolean acceptOrDecline = ConsoleUI.promptForBool(whatPlayerToTradeWith(inputPlayerNumber, player) + "would you like to trade with the player intiating trade?" + "[Y/N]","Y", "N");

		if (acceptOrDecline == true) {
			boolean willYouModifyTrade = ConsoleUI.promptForBool("Would you like to modify your trade?" + "[Y/N]", "Y", "N");
			...
			if (willYouModifyTrade == true) {
					...
					if (modifyExchange == true) {
						...
						while (!checkModifiedAmount) {
							if (enter2Value > Player.getMoney(player)) {
								...
								System.out.println(whatPlayerToTradeWith(inputPlayerNumber, player) + " has " + enterValue + " added to their account");
				...
				if (selectWhatToModify == 2) {
					...
					if (selectWhatToModify == 3) {
						boolean willYouTradePlayerProperty = ConsoleUI.promptForBool("Would you like to trade some property?" + "[Y/N]", "Y", "N");
							...
							while (isTradeTrue) {
								...
								if (continuePropTrade == true) {
									System.out.println("What property do you want?");
									System.out.println(Player.getland(Game.getPlayerWhosTurnItIs()));
									int tradeLand = ConsoleUI.promptForInt("Select what property DO you want to take", 1,Integer.MAX_VALUE);
									tradingLand = Player.getland(Game.getPlayerWhosTurnItIs());
