...
public class Trade {
	...
	public static Player whatPlayerToTradeWith(int inputPlayerNumber, Player player) throws IOException {
		...
		boolean pickedThemself = false;
		do {
			pickedThemself = false;
			inputPlayerNumber = ConsoleUI.promptForInt("Select player to trade with", 1, 8);
			Game.returnPlayerOne();
			if(inputPlayerNumber == 1 && (Game.returnPlayerOne() == Game.getPlayerWhosTurnItIs()) && Player.getPlayingGame(Game.returnPlayerOne())){
				playerToTradeWith = Game.returnPlayerOne();
			} else if(inputPlayerNumber == 2 && (Game.returnPlayerTwo() == Game.getPlayerWhosTurnItIs()) && Player.getPlayingGame(Game.returnPlayerTwo())){
				playerToTradeWith = Game.returnPlayerTwo();
			} else if(inputPlayerNumber == 3 && (Game.returnPlayerThree() == Game.getPlayerWhosTurnItIs()) && Player.getPlayingGame(Game.returnPlayerThree())){
				playerToTradeWith = Game.returnPlayerThree();
			} else if(inputPlayerNumber == 4 && (Game.returnPlayerFour() == Game.getPlayerWhosTurnItIs()) && Player.getPlayingGame(Game.returnPlayerFour())){
				playerToTradeWith = Game.returnPlayerFour();
			} else if(inputPlayerNumber == 5 && (Game.returnPlayerFive() == Game.getPlayerWhosTurnItIs()) && Player.getPlayingGame(Game.returnPlayerFive())){
				playerToTradeWith = Game.returnPlayerFive();
			} else if(inputPlayerNumber == 6 && (Game.returnPlayerSix() == Game.getPlayerWhosTurnItIs()) && Player.getPlayingGame(Game.returnPlayerSix())){
				playerToTradeWith = Game.returnPlayerSix();
			} else if(inputPlayerNumber == 7 && (Game.returnPlayerSeven() == Game.getPlayerWhosTurnItIs()) && Player.getPlayingGame(Game.returnPlayerSeven())){
				playerToTradeWith = Game.returnPlayerSeven();
			} else if(inputPlayerNumber == 8 && (Game.returnPlayerEight() == Game.getPlayerWhosTurnItIs()) && Player.getPlayingGame(Game.returnPlayerEight())){
	...
	public static int askCurrentPlayerForTrade() throws IOException {
		...
		if (menuSelect == 1) {
			...
			if (selectWhatToTrade == 1) {
				boolean moneyExchange = ConsoleUI.promptForBool("Would you like to exchange some cash?" + "[Y/N]", "Y",
						"N");

				if (moneyExchange == true) {
					enterValue = ConsoleUI.promptForInt("Enter in the value of money you would like to trade", 0,
							Integer.MAX_VALUE);
			...
			if (selectWhatToTrade == 2) {
				boolean tradeJailCard = ConsoleUI
				...
				if (tradeJailCard == true) {
					...
					currentJailCard = Player.getGetOutOfJailChance(Game.getPlayerWhosTurnItIs())
			...
			if (selectWhatToTrade == 3) {
				boolean tradeProperty = ConsoleUI.promptForBool("Would you like to trade some property?" + "[Y/N]", "Y",
						"N");
				if (tradeProperty == true) {
					...
					while (tradeCon) {
						...
						if (continuePropTrade == true) {
							...
							int tradeLand = ConsoleUI.promptForInt(
									"Select what property you want to remove from your inventory", 1,
									tradingLand.size());
	...
	public static int selectWhatOtherPlayerTrades() throws IOException {
		...
		if (selectPlayerTrade == 1) {
			boolean moneyPlayerExchange = ConsoleUI
					.promptForBool("What would you like for the other player to exchange?" + "[Y/N]", "Y", "N");
			if (moneyPlayerExchange == true) {
				...
				if (otherPlayersMoney < enter2Value) {
					System.out.println(
							"Error! " + player + " doesn't have that money money in their account. Try Again.");
				} else {
		...
		if (selectPlayerTrade == 3) {
			boolean tradePlayerProperty = ConsoleUI.promptForBool("Would you like to request some property?" + "[Y/N]",
					"Y", "N");
	...
	public static boolean acceptOrDeclinePlayerTradeRequest() throws IOException {
		...
		boolean acceptOrDecline = ConsoleUI.promptForBool(
				player + "Would you like to trade with " + Game.getPlayerWhosTurnItIs() + " intiating trade?" + "[Y/N]",
				"Y", "N");
		...
		if (acceptOrDecline == true) {
			...
			if (modifyOrNah == true) {
				...
				if (selectWhatToModify == 1) {
					boolean wannaExchange = ConsoleUI.promptForBool("What would you like to exchange money?" + "[Y/N]",
							"Y", "N");
				...
				if (selectWhatToModify == 2) {
					...
					if (modifyJailCard == true) {
						...
						if (currentJailCard2 = false) {
							System.out.println(
									"Sorry, " + player + "  you still don't possess a Get Out of Jail Free card");
						}

						// else if (Player.getGetOutOfJailChance(player) ||
						// Player.getGetOutOfJailChest(player))
					...
					if (selectWhatToModify == 3) {
						boolean wannaTradePlayerProperty = ConsoleUI
